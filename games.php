 ...
 <?php include("head.html"); ?>
    <body>
        ...
        if(isset($_POST["search"])){
            ...
            if($_POST["view"] == "all"){
                  //get total quantity
                $sum = "SELECT sum(quantity) FROM games WHERE $criteria LIKE ? AND username = ? ORDER BY title";
            ...
            //get total quantity
            $sum = "SELECT sum(quantity) FROM games WHERE username = ? ORDER BY title";
            $stmt = $conn->prepare($sum);
        ...
        if ($result->num_rows > 0) {
            ...
            echo "</table>";
            //<a href='edit_games.php?id=$ID'>Edit</a>
        } else {
