...
<html lang="en">
 <?php include("head.html"); ?>
    <body>
        ...
        <h2><?php echo "$user's Accessories"; ?></h2>
        ...
        if(isset($_POST["search"])){
            ...
            if($_POST["view"] == "all"){
                //get total quantity
                $sum = "SELECT sum(quantity) FROM accessories WHERE $criteria LIKE ? AND username = ? ORDER BY accessoryName";
            ...
            $sum = "SELECT sum(quantity) FROM accessories WHERE username = ? ORDER BY accessoryName";
            $stmt = $conn->prepare($sum);
            ...
            while($row = $result->fetch_assoc()) {
                $ID = $row["accessoryID"];
                 echo "<tr>". 
                         ...
                         "<td>" . $row["comments"] . "</td>" .
                         "<td>" . 
                           "<form method='post' action='edit_accessories.php'>
